function varargout = edgesEvalDir_x( varargin )
...
% *********** important for dataset evaluation ***********
% perform evaluation on each image (this part can be very slow)
assert(exist(resDir,'dir')==7); assert(exist(gtDir,'dir')==7);
ids=dir(fullfile(gtDir,'*.png')); ids={ids.name}; n=length(ids); % gt names
do=true(1,n); jobs=cell(1,n); res=cell(1,n);
for i=1:n, id=ids{i}(1:end-4);
  res{i}=fullfile(evalDir,[id '_ev1.txt']); do(i)=~exist(res{i},'file');
  im1=fullfile(resDir,[id '.png']); gt1=fullfile(gtDir,[id '.png']);
  disp(im1);
  disp(gt1);
  disp(n);
  disp(i);
  jobs{i}={im1,gt1,'out',res{i},'thrs',p.thrs,'maxDist',p.maxDist,...
    'thin',p.thin}; if(0), edgesEvalImg_x(jobs{i}{:}); end
end
fevalDistr('edgesEvalImg_x',jobs(do),p.pDistr{:});
